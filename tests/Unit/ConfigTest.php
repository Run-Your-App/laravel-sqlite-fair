...
it('owns exactly the three package defaults', function () {
    $defaults = require dirname(__DIR__, 2).'/config/sqlite-fair.php';

    expect($defaults)->toHaveCount(3)
        ->and(array_keys($defaults))->toBe(['lock_directory', 'stale_head_seconds', 'wait_strategy'])
