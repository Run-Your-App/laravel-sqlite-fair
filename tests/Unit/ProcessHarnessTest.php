...
it('retains only the newest completed runs without deleting current or locked runs', function (): void {
    $root = $GLOBALS['sqliteFairTestRunDirectory'].DIRECTORY_SEPARATOR.'retention-proof';
    ...
    foreach ($completed as $directory) {
        mkdir($root.DIRECTORY_SEPARATOR.$directory, 0775);
        touch($root.DIRECTORY_SEPARATOR.$directory.DIRECTORY_SEPARATOR.'database.sqlite');
    }
    $legacyCompleted = $root.DIRECTORY_SEPARATOR.'20250101.000000.000';
    mkdir($legacyCompleted, 0775);
    touch($legacyCompleted.DIRECTORY_SEPARATOR.'database.sqlite');
    $locked = $root.DIRECTORY_SEPARATOR.'20260101.000000.000.eeeeeeee';
    mkdir($locked, 0775);
    $lockedHandle = fopen($locked.DIRECTORY_SEPARATOR.'.active', 'c+');
    ...
    $current = $root.DIRECTORY_SEPARATOR.'20260101.000005.000.ffffffff';
    mkdir($current, 0775);
    ...
    try {
        ...
        expect(is_dir($current))->toBeTrue()
            ...
            ->and(is_dir($root.DIRECTORY_SEPARATOR.$completed[3]))->toBeTrue()
            ->and(is_dir($root.DIRECTORY_SEPARATOR.$completed[2]))->toBeTrue()
            ->and(is_dir($root.DIRECTORY_SEPARATOR.$completed[1]))->toBeFalse()
            ->and(is_dir($root.DIRECTORY_SEPARATOR.$completed[0]))->toBeFalse()
